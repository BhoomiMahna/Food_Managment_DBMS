...
export default function WholesalerDashboard() {
    ...
    const [farmersStock, setFarmersStock] = useState([]);
    ...
    const fetchData = async () => {
        try {
            const farmersRes = await axios.get(`http://localhost:5000/wholesaler/farmers${regionFilter ? `?city=${regionFilter}` : ''}`);
            setFarmersStock(farmersRes.data);
            const myRes = await axios.get('http://localhost:5000/wholesaler/stock');
            setMyStock(myRes.data);
            const citiesRes = await axios.get('http://localhost:5000/cities');
            setCities(citiesRes.data.filter(c => c.source === 'farmer'));
        } catch (err) {
            console.error(err);
        }
    };
    ...
    const handleConfirmPurchase = async (qty, markup) => {
        try {
            await axios.post('http://localhost:5000/wholesaler/buy', {
                crop_id: selectedItem.crop_id,
        ...
        <div className="min-h-screen p-8 fade-in">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex justify-between items-center glass p-8">
                    <div>
                        <h1 className="text-3xl font-black text-white tracking-tighter">Wholesaler Dashboard</h1>
                ...
                <div className="glass p-8 min-h-[500px]">
                    <div className="mb-8 border-b border-white/5 pb-6">
                        <h2 className="text-xl font-black text-white uppercase tracking-tighter">
                            {activeTab === 'market' ? 'Available Crops' : 'Your Inventory'}
                        </h2>
                        <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mt-1">Live stock records from the network</p>
                    </div>
                    
                    {activeTab === 'market' ? (
                        <StockTable 
                            data={filterData(farmersStock)} 
                            onAction={handleActionClick} 
                            isWholesalerView={true} 
                        />
                    ) : (
